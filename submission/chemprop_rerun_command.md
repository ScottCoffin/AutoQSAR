...
Verified locally: 4/4 regression assertions pass, the 0xe2 decode failure is reproduced byte-for-byte,
...
Once step 1 is clean. One canonical D-MPNN configuration is preferable to five partially working
variants; the peer review asked for this explicitly, and the RTX evidence (§5) shows the extra
variants buy nothing. `--run-chemprop-rdkit2d` is the ADMET-AI-equivalent variant and is the one that
...
nohup python portable_colab_qsar_bundle/run_qsarena_benchmarks.py \
  ...
  --no-run-chemprop-attentivefp --no-run-chemprop-selected-features \
  --no-run-unimol-v1 \
  --run-cfa --run-ensemble --rebuild-ensemble \
...
- `--no-run-unimol-v1` skips the most expensive family (median 374 s/model, ≈55× conventional). Uni-Mol
  results are unchanged by either fix, so they are carried over from the deposited run at analysis
  time rather than recomputed. **If you would rather have one internally consistent run, drop this
  flag** and accept the full ≈112 h cost.
